...
const brands = [
  ...
  { name: "Premium Shocks", logo: premiumShocksLogo, height: "h-5 md:h-6" },
  { name: "RC Automotive", logo: rcAutoLogo, height: "h-5 md:h-6" },
    ...
    <div className="min-h-screen text-white overflow-hidden relative bg-[#050508] flex flex-col">
      ...
      {!gameActive && (
        <button
          ...
          className="md:hidden fixed bottom-20 right-3 z-30 bg-black/80 backdrop-blur-xl rounded-full px-3 py-2 border border-yellow-500/30 shadow-lg flex items-center gap-1.5"
        >
      ...
      {showLeaderboardModal && !gameActive && (
        <div className="md:hidden fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setShowLeaderboardModal(false)}>
          <div className="bg-black/95 backdrop-blur-xl rounded-2xl border border-white/10 w-[85%] max-w-sm overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="bg-gradient-to-r from-yellow-500/20 to-purple-500/20 px-4 py-3 border-b border-white/10 flex items-center justify-between">
              ...
              <button onClick={() => setShowLeaderboardModal(false)} className="text-white/50 hover:text-white text-lg">✕</button>
            </div>
      ...
      {/* Desktop: Floating Leaderboard panel */}
      {!gameActive && (
        <div className="hidden md:block fixed bottom-20 right-4 z-30 w-52">
          ...
          {showLevelUp && (
            <div className="absolute inset-0 flex items-center justify-center z-20 bg-black/80 backdrop-blur-sm">
              <div className="text-center">
                ...
                {/* Show how many reached this level */}
                {(level + 1) >= 3 && (
                  <div className={`mt-4 px-4 py-2 rounded-full inline-block ${(level + 1) === 5 ? 'bg-yellow-500/20 border border-yellow-500/40' : (level + 1) === 4 ? 'bg-purple-500/20 border border-purple-500/40' : 'bg-white/10'}`}>
                    <p className={`text-sm font-bold ${(level + 1) === 5 ? 'text-yellow-400' : (level + 1) === 4 ? 'text-purple-400' : 'text-white/60'}`}>
                      Only {formatCount(levelStats[level + 1] || 0)} players have reached this level!
                    </p>
                  </div>
                )}
              </div>
